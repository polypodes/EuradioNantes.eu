...
class PodcastProvider extends BaseProvider
{
    ...
    protected function mp32OggFile($file, $delete = FALSE)
    {
    	...
    	if(file_exists("/var/www/euradionantes.prod/httpdocs/web/uploads/media/$path/$fileName"))
    	{
    		$res = @system("ffmpeg -i /var/www/euradionantes.prod/httpdocs/web/uploads/media/$path/$fileName -vcodec libtheora -acodec libvorbis /var/www/euradionantes.prod/httpdocs/web/uploads/media/$path/$fileName.ogg");
    		if($delete == TRUE)
    		{
    			//unlink($file);
    		}
